...
(function($){
    $.slickQuiz = function(element, options) {
        ...
        plugin.method = {
            ...
            setupQuiz: function() {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        questionHTML.append('<h1 class="questionCount styled-header">Question <span class="current">' + count + '</span> of <span class="total">' + questionCount + '</span></h1>');
                        if (options.count) {
                        ...
                        var answerHTML = $('<ul class="answers ' + (truths > 1 ? 'checkbox' : 'radio') + '"></ul>');
            ...
            completeQuiz: function() {
                ...
                $(targets.quizArea).fadeOut(300, function() {
				    ...
				    if (pagetype=='assessments') {
						user.suspend_data.assessments[lessonNumber].slides[currentPage].score = percent;
				        user.lesson_score = percent * 100;
				        if (levelRank==1) {
						    user.suspend_data.assessments[lessonNumber].slides[currentPage].passed = 'passed';
						    user.suspend_data.lesson_status='passed';
						    user.lesson_status='passed';
						    $(targets.quizResultsCopy).text('Congratulations. You have passed this module. Click close to return to the course map.');
				        } else {
						    user.suspend_data.assessments[lessonNumber].slides[currentPage].passed = 'incomplete';
						    user.suspend_data.lesson_status='incomplete';
						    user.lesson_status='incomplete';
						    $(targets.quizResultsCopy).text('Sorry. You did not pass this module. Please review the module and try again. Click close to return to the course map.');
						}
						//checkAssessment();
				        saveUser();
				    }
            ...
            calculateLevel: function(correctAnswers) {
                ...
                if (plugin.method.inRange(0, 0.20, percent)) {
                    ...
                    level = 3;
                } else if (plugin.method.inRange(0.61, 0.79, percent)) {
                    level = 2;
        ...
        plugin.init = function() {
            ...
            $(triggers.checker).click(function(e) {
                e.preventDefault();
                plugin.method.checkAnswer(this);
            });

            // Bind "back" button
            $(triggers.back).click(function(e) {
                e.preventDefault();
                plugin.method.backToQuestion(this);
            });

            // Bind "next question" button
            $(triggers.next).click(function(e) {
                e.preventDefault();
